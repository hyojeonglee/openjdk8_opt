...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv, void *args) {
	...
	//
#if defined(ZERO) && defined(ASSERT) // added by charlie : not defined
	{
		jint a = 0xcafebabe;
	...
	if (Atomic::xchg(1, &vm_created) == 1) { //added by charlie : to make sure only one thread can create the VM
